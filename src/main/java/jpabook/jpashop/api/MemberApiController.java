 ...
 * -> @ResponseBody : 데이터를 JSON의 XML로 보내준다
 */
@RestController
...
public class MemberApiController {
    ...
    /**
     * Response JSON 이 나갈때, JSON Array 가 바로 나가면 나중에 확장하기 어렵다
     * -> "data" : [] 처럼 data 필드에 한번 감싸서 내보내는게 확장성면에서 좋다
     */
    @Data
    @AllArgsConstructor
     ...
     * - 엔티티가 변경되면 API 스펙이 변한다. shit!
     * 결론
     * - API 요청 스펙에 맞추어 별도의 DTO를 파라미터로 받는다.
     */
    @PostMapping("/api/v1/members")
    public CreateMemberResponse saveMemberV1(@RequestBody @Valid Member member) {
        // @RequestBody Member member -> JSON 으로 온 Body 를 Member 객체에 매핑해준다
     ...
     * -> Command : memberService.update
     * -> Query : memberService.findOne
     */
